...
main(int argc, char **argv)
{
	...
	LXAArchive *archive_gz = lxa_new_archive("./composite-archives/archive.tar.gz", LXA_ARCHIVETYPE_TAR, LXA_COMPRESSIONTYPE_GZIP);
	if(archive_gz == NULL)
	{
		g_critical("Could not create a new archive");
		return 1;
	...
	ret = lxa_archive_add(archive_gz, files);

	g_signal_connect(G_OBJECT(archive_tbz2), "lxa_status_changed", G_CALLBACK(test_archive_status_changed), NULL);
	g_signal_connect(G_OBJECT(archive_gz), "lxa_status_changed", G_CALLBACK(test_archive_status_changed), NULL);
